...
export enum Verbs {    GET = 'GET',    PUT = 'PUT',    POST = 'POST',    DELETE = 'DELETE'}

@Injectable({  providedIn: 'root'})
export class HttpclientService {

  constructor( private http: HttpClient, private cacheclientService: CacheclientService) { }
...
private httpCall<T>(verb: Verbs, options: HttpOptions): Observable<T> {
    ...
    if (options.cacheMins > 0) {
        // Get data from cache
        const data = this.cacheclientService.load(options.url);
